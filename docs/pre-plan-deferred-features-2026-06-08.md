...
These were the explicit *post-v1* deferrals at the end of the original phase 1–4 effort that produced the agent-playground codebase (the work now parked on [`wip/2026-06-01-pre-adoption`](https://github.com/dragonflowio/agent-playground/tree/wip/2026-06-01-pre-adoption)). The planning was captured in `docs/agent-playground/future-tasks.md` inside `Dragonflow_Core` while this app still lived as `Dragonflow_Core/apps/agent-playground/`. That folder was left untracked when the app moved to its own repo, and the rest of those plan docs were retired in 2026-06-08 alongside the playbook adoption. This file preserves the deferred-feature list — the only piece of the legacy docs with forward value — so it can be triaged into real plans alongside the `wip/2026-06-01-pre-adoption` work.
